...
public class ProceduralLegPlacement : MonoBehaviour {
    public float dynamicStep = 1;

    public Vector3 optimalRestingPosition = Vector3.forward;
    public Vector3 restingPosition {
        get {
            return transform.TransformPoint (optimalRestingPosition) - worldVelocity;
        }
    }
    ...
    public void UpdateIkTarget () {
        ...
        ikTarget.position = Vector3.Lerp (ikTarget.position, worldTarget + worldVelocity * dynamicStep, percent) + Vector3.up * stepHeightCurve.Evaluate (percent) * stepHeightMultiplier;
    }
